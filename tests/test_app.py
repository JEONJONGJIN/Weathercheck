...
class HelpersTests(unittest.TestCase):
    ...
    def test_active_providers_for_app_includes_accuweather(self) -> None:
        original_key = os.environ.get("ACCUWEATHER_API_KEY")
        try:
            os.environ["ACCUWEATHER_API_KEY"] = "accu-key"
            providers = app.active_providers_for_app()
            provider_names = [name for name, _ in providers]
            self.assertIn("AccuWeather", provider_names)
        finally:
            if original_key is None:
                os.environ.pop("ACCUWEATHER_API_KEY", None)
            else:
                os.environ["ACCUWEATHER_API_KEY"] = original_key
